...
export const PlaylistsContext = createContext({
    ...
    deleteOldCategories: ({fileId, plyalistId, selectedOld}) => {},
...
const PlaylistsContextProvider = ({ children }) => {
        ...
        const deleteCategories = (selectedCategories) => {
            const selectedToLower = selectedCategories.map(cat => {
                return cat.toLowerCase();
            });

            const updatedCats = categories.filter(cat => {
                return !selectedToLower.includes(cat)
            });
            setCategories(updatedCats);
        }

        // delete categories previously assigned to a file
        const deleteOldCategories = ({fileId, playlistId, selectedOld}) => {
                  ...
                  return {
                      ...
                      linkedFiles: playlist.linkedFiles.map(file => {
                        if (file.id === fileId) {
                          const updatedFile = {
                            ...file,
                            category: file.category.filter(cat => {
                              return !selectedOld.includes(cat);
                           }),
                            image: {...file.image}
                          }
                          return updatedFile;
